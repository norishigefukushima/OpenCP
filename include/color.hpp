...
namespace cp
{
	...
	CP_EXPORT double cvtColorPCAErrorPSNR(const std::vector<cv::Mat>& src, const int dest_channels, const bool isShow = false, const int rows = 1);
	CP_EXPORT double cvtColorPCAErrorPSNR(const cv::Mat& src, const int dest_channels, const bool isShow = false, const int rows = 1);
