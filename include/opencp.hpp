...
#define COLOR_CYAN cv::Scalar(255,255,0)
	...
	//slic
	CP_EXPORT void SLICSegment2Vec(cv::InputArray segment, std::vector<std::vector<cv::Point>>& segmentPoint);
	...
	CP_EXPORT void maxFilter(const cv::Mat& src, cv::Mat& dest, cv::Size kernelSize, int shape = cv::MORPH_RECT);
	CP_EXPORT void maxFilter(const cv::Mat& src, cv::Mat& dest, int radius);
	//MORPH_RECT=0, MORPH_CROSS=1, MORPH_ELLIPSE
	CP_EXPORT void minFilter(const cv::Mat& src, cv::Mat& dest, cv::Size kernelSize, int shape = cv::MORPH_RECT);
	CP_EXPORT void minFilter(const cv::Mat& src, cv::Mat& dest, int radius);
