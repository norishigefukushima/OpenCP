...
namespace cp
{
	void MultiScaleBilateralFilter::buildDoBFStack(const Mat& src, vector<Mat>& DoBFStack, const float sigma_r, const float sigma_s, const int level)
	{
		...
		for (int l = 1; l <= level; l++)
		{
			...
			cp::bilateralFilter(src, DoBFStack[l], ksize, sigma_r, sigma_l, FILTER_RECTANGLE, borderType);
			//guiAlphaBlendScale(src, DoBFStack[l]);
			subtract(prev, DoBFStack[l], DoBFStack[l - 1]);
			prev = DoBFStack[l];
		}
	}

	void MultiScaleBilateralFilter::buildCoBFStack(const Mat& src, vector<Mat>& DoBFStack, const float sigma_r, const float sigma_s, const int level)
	{
		if (DoBFStack.size() != level + 1) DoBFStack.resize(level + 1);
		for (int l = 0; l <= level; l++) DoBFStack[l].create(src.size(), CV_32F);
		...
		for (int l = 1; l <= level; l++)
		{
			...
			cp::bilateralFilter(src, DoBFStack[l], ksize, sigma_r, sigma_l, FILTER_RECTANGLE, borderType);
			//guiAlphaBlendScale(src, DoBFStack[l]);
			divide(prev, DoBFStack[l] + FLT_EPSILON, DoBFStack[l - 1]);
			DoBFStack[l - 1] -= 1.f;
			prev = DoBFStack[l];
		}
	}
	...
	{
		cout << "not support: MultiScaleBilateralFilter::pyramid" << endl;
	...
	void MultiScaleBilateralFilter::cog(const Mat& src, Mat& dest)
	{
		//cout << "BF DoG" << endl;
		ImageStack.resize(level + 1);
		initRangeTable(sigma_range, boost);
		...
		buildCoBFStack(src, ImageStack, sigma_range, sigma_space, level);
		for (int i = 0; i < ImageStack.size() - 1; i++)
		{
			remap(ImageStack[i], ImageStack[i], 0.f, sigma_range, boost);
		}
