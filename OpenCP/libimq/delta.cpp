...
for(_INT32 pos = 0; pos < size; ++pos)  tmp += (_INT64)orig_img[pos] - (_INT64)comp_img[pos];
	
Delta = double(tmp) / double(size);
...
switch(BPP) {
	...
	case 8: {
		...
		result = Delta8bit(orig_imgb,comp_imgb,PX,PY);
		delete [] orig_imgb;
		delete [] comp_imgb;
	...
	case 16: {
		...
		result = Delta16bit(orig_imgb,comp_imgb,PX,PY);
		delete [] orig_imgb;
		delete [] comp_imgb;
...
double DoDeltaY(_INT32 *orig_img, _INT32 *comp_img, _INT32 PX, _INT32 PY, _INT32 BPP)
{
	...
	case 8: case 16:
				result = DoDelta(orig_img, comp_img, PX,PY,BPP);
				break;
