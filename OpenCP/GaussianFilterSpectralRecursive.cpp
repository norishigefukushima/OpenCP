		...
		public:
			...
			void filter(const cv::Mat& src, cv::Mat& dst)
			{
				...
				case CV_32FC1:
					...
					break;
				case CV_64FC1:
					filter<double>(src.cols, src.rows, reinterpret_cast<double*>(src.data), reinterpret_cast<double*>(dst.data));
					break;
				default:
					throw std::invalid_argument("Unsupported element type!");
					break;
...
#define _MM_TRANSPOSE2_PD(row0, row1) {          \
	...
	void GaussianBlurSR_cliped_32F(InputArray src_, OutputArray dest, float sigma)
	{
		...
		else if (dest.depth() != CV_32F) srcf2.convertTo(dest, CV_32F);
