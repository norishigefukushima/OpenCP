...
void jointNonLocalMeansFilterBase(Mat& src, Mat& guide, Mat& dest, int templeteWindowSize, int searchWindowSize, double h, double sigma)
{
	...
	copyMakeBorder(guide,gim,bb,bb,bb,bb,cv::BORDER_DEFAULT);
...
void jointNonLocalMeansFilter(Mat& src, Mat& guide, Mat& dest, Size templeteWindowSize, Size searchWindowSize, double h, double sigma)
{
	...
	if(guide.channels()==1)
	{
		copyMakeBorder(guide,gim,bby,bby,bbx,bbx+spad,cv::BORDER_DEFAULT);
	}
	else if (guide.channels()==3)
	{
		Mat temp;
		copyMakeBorder(guide,temp,bby,bby,bbx,bbx+spad,cv::BORDER_DEFAULT);
...
{
	jointNonLocalMeansFilter(src, guide, dest, Size(templeteWindowSize, templeteWindowSize), Size(searchWindowSize,searchWindowSize), h, sigma);
}
