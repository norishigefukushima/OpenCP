...
namespace tiling
{
	guidedFilter_Merge_nonVec::guidedFilter_Merge_nonVec(cv::Mat _src, cv::Mat _guide, cv::Mat& _dest, int _r, float _eps, int _parallelType)
	...
	void RowSumFilter_Ip2ab_Guide1::filter_naive_impl()
	{
		for (int j = 0; j < img_row; j++)
		{
			...
			//float* v0_p = tempVec1[0].ptr<float>(j); // mean_I
			//float* v1_p = tempVec1[1].ptr<float>(j); // mean_p
			//float* v2_p = tempVec1[2].ptr<float>(j); // corr_I
			//float* v3_p = tempVec1[3].ptr<float>(j); // corr_Ip
			float* v0_p = tempVec[0].ptr<float>(j) + r; // mean_I
			float* v1_p = tempVec[1].ptr<float>(j) + r; // mean_p
			float* v2_p = tempVec[2].ptr<float>(j) + r; // corr_I
			float* v3_p = tempVec[3].ptr<float>(j) + r; // corr_Ip
...
#pragma omp parallel for
	...
	void ColumnSumFilter_Ip2ab_Guide1_nonVec::filter_naive_impl()
	{
		...
		for (int i = r; i < img_col - r; i++)
		{
			...
			float* a_p = a.ptr<float>(r) + i;
			float* b_p = b.ptr<float>(r) + i;
...
#pragma omp parallel for
	...
	void ColumnSumFilter_Ip2ab_Guide1_AVX::filter_naive_impl()
	{
		for (int i = 0; i < img_col; i++)
		{
			...
			float* a_p = a.ptr<float>(r) + 8 * i;
			float* b_p = b.ptr<float>(r) + 8 * i;
			...
			__m256 mSum0 = _mm256_load_ps(v0_p1);
			__m256 mSum1 = _mm256_load_ps(v1_p1);
			__m256 mSum2 = _mm256_load_ps(v2_p1);
			__m256 mSum3 = _mm256_load_ps(v3_p1);
...
#pragma omp parallel for
	...
	{
		//for (int j = 0; j < img_row; j++)
		for (int j = r; j < img_row - r; j++)
		{
			float* a_p1 = a.ptr<float>(j) + r;
			float* b_p1 = b.ptr<float>(j) + r;
			float* a_p2 = a.ptr<float>(j) + r + 1;
			float* b_p2 = b.ptr<float>(j) + r + 1;
			float* v0_p = tempVec[0].ptr<float>(j) + 2 * r;
			float* v1_p = tempVec[1].ptr<float>(j) + 2 * r;
			//float* a_p1 = a.ptr<float>(j);
			//float* b_p1 = b.ptr<float>(j);
			//float* a_p2 = a.ptr<float>(j) + 1;
			//float* b_p2 = b.ptr<float>(j) + 1;
			//float* v0_p = tempVec1[0].ptr<float>(j);
			//float* v1_p = tempVec1[1].ptr<float>(j);
...
#pragma omp parallel for
	...
	void ColumnSumFilter_ab2q_Guide1_nonVec::filter_naive_impl()
	{
		...
		for (int i = r; i < img_col - r; i++)
		{
			float* v0_p1 = tempVec[0].ptr<float>(0 + r) + i;
			float* v1_p1 = tempVec[1].ptr<float>(0 + r) + i;
			float* v0_p2 = tempVec[0].ptr<float>(1 + r) + i;
			float* v1_p2 = tempVec[1].ptr<float>(1 + r) + i;
			float* I_p = I.ptr<float>(2 * r) + i;
			float* q_p = q.ptr<float>(2 * r) + i;
			//float* v0_p1 = tempVec[0].ptr<float>(0) + i;
			//float* v1_p1 = tempVec[1].ptr<float>(0) + i;
			//float* v0_p2 = tempVec[0].ptr<float>(1) + i;
			//float* v1_p2 = tempVec[1].ptr<float>(1) + i;
...
#pragma omp parallel for
	...
	void ColumnSumFilter_ab2q_Guide1_AVX::filter_naive_impl()
	{
		for (int i = 0; i < img_col; i++)
		{
			float* v0_p1 = tempVec[0].ptr<float>(0 + r) + i * 8;
			float* v1_p1 = tempVec[1].ptr<float>(0 + r) + i * 8;
			float* v0_p2 = tempVec[0].ptr<float>(1 + r) + i * 8;
			float* v1_p2 = tempVec[1].ptr<float>(1 + r) + i * 8;
			float* I_p = I.ptr<float>(2 * r) + i * 8;
			float* q_p = q.ptr<float>(2 * r) + i * 8;
			...
			__m256 ma = _mm256_load_ps(v0_p1);
			__m256 mb = _mm256_load_ps(v1_p1);
			...
			__m256 mq = _mm256_add_ps(_mm256_mul_ps(mI, ma), mb);
