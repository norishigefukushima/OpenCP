...
namespace cp
{
#pragma region binaryWeightedRangeFilter
	//for test, non SSE code for tbb
	class binaryWeightedRangeFilter_8u_Invoker : public cv::ParallelLoopBody
	{
	public:
		binaryWeightedRangeFilter_8u_Invoker(Mat& _dest, const Mat& _temp, int _radiusH, int _radiusV, int _maxk,
	...
	void binaryWeightedRangeFilterNONSSE_8u(const Mat& src, Mat& dst, Size kernelSize, uchar threshold, int borderType, bool isRectangle)
	{
		...
		binaryWeightedRangeFilter_8u_Invoker body(dest, temp, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	//for test, non SSE code for tbb
	class binaryWeightedRangeFilter_32f_Invoker : public cv::ParallelLoopBody
	{
	public:
		binaryWeightedRangeFilter_32f_Invoker(Mat& _dest, const Mat& _temp, int _radiusH, int _radiusV, int _maxk,
	...
	void binaryWeightedRangeFilterNONSSE_32f(const Mat& src, Mat& dst, Size kernelSize, float threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		if (dst.empty())dst = Mat::zeros(src.size(), src.type());
		int cn = src.channels();
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		binaryWeightedRangeFilter_32f_Invoker body(dest, temp, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	class binaryWeightedRangeFilter_8u_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		binaryWeightedRangeFilter_8u_InvokerSSE4(Mat& _dest, const Mat& _temp, int _radiusH, int _radiusV, int _maxk,
			int* _space_ofs, uchar _threshold) :
			temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV),
			maxk(_maxk), space_ofs(_space_ofs), threshold(_threshold)
		{
	...
	void binaryWeightedRangeFilter_8u(const Mat& src, Mat& dst, Size kernelSize, uchar threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int i, j, maxk;
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		binaryWeightedRangeFilter_8u_InvokerSSE4 body(dest, temp, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	void binaryWeightedRangeFilterSP_8u(const Mat& src, Mat& dst, Size kernelSize, uchar threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else binaryWeightedRangeFilter_8u(src, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			binaryWeightedRangeFilter_8u(dst, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}

	class binaryWeightedRangeFilter_32f_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		binaryWeightedRangeFilter_32f_InvokerSSE4(Mat& _dest, const Mat& _temp, int _radiusH, int _radiusV, int _maxk,
			int* _space_ofs, float _threshold) :
			temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV),
			maxk(_maxk), space_ofs(_space_ofs), threshold(_threshold)
		{
	...
	void binaryWeightedRangeFilter_32f(const Mat& src, Mat& dst, Size kernelSize, float threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int i, j, maxk;
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		binaryWeightedRangeFilter_32f_InvokerSSE4 body(dest, temp, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	void binaryWeightedRangeFilterSP_32f(const Mat& src, Mat& dst, Size kernelSize, float threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else binaryWeightedRangeFilter_32f(src, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			binaryWeightedRangeFilter_32f(dst, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}


	class binaryWeightedRangeFilter_16S_InvokerAVX : public cv::ParallelLoopBody
	{
	public:
		binaryWeightedRangeFilter_16S_InvokerAVX(Mat& _dest, const Mat& _temp, int _radiusH, int _radiusV, int _maxk,
			int* _space_ofs, float _threshold) :
			temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV),
			maxk(_maxk), space_ofs(_space_ofs), threshold(_threshold)
		{
		...
		virtual void operator() (const Range& range) const
		{
			...
			const __m256i mth = _mm256_set1_epi32((int)threshold);
			if (cn == 1)
			{
				short* sptr = (short*)temp->ptr<short>(range.start + radiusV) + 8 * (radiusH / 8 + 1);
				short* dptr = dest->ptr<short>(range.start);

				const int sstep = temp->cols;
				const int dstep = dest->cols;
				const int SIZE = get_simd_floor(size.width, 8);
				for (int i = range.start; i != range.end; i++, dptr += dstep, sptr += sstep)
				{
					int j = 0;					
					for (; j < SIZE; j += 8)//8 pixel unit
					{
						int* ofs = space_ofs;

						const short* sptrj = sptr + j;
						const __m256i sval0 = _mm256_cvtepi16_epi32(_mm_loadu_si128((__m128i*)sptrj));

						__m256i tval = _mm256_setzero_si256();
						__m256i wval = _mm256_set1_epi32(maxk);
						for (int k = 0; k < maxk; k++)
						{
							const __m256i sref = _mm256_cvtepi16_epi32(_mm_loadu_si128((__m128i*)(sptrj + *ofs++)));
							const __m256i w = _mm256_cmpgt_epi32(_mm256_abs_epi32(_mm256_sub_epi32(sval0, sref)), mth);
							tval = _mm256_add_epi32(tval, _mm256_andnot_si256(w, sref));
							wval = _mm256_add_epi32(wval, w);
						}
						tval = _mm256_div_epi32(tval, wval);
						_mm_storeu_si128((__m128i*)(dptr + j), _mm_packs_epi32(_mm256_castsi256_si128(tval), _mm256_castsi256hi_si128(tval)));
					}
					for (; j < size.width; j++)
					{
						const float val0 = sptr[j];
						...
						dptr[j] = saturate_cast<short>(sum / wsum);
					}
				}
			}
			else
			{
				//underconstruction
				cout << "not implemented binaryWeightedRangeFilter_16S_InvokerAVX" << endl;
				const int sstep = 3 * temp->cols;
				const int dstep = dest->cols * 3;
				short* sptrb = (short*)temp->ptr(3 * radiusV + 3 * range.start) + 4 * (radiusH / 4 + 1);
				short* sptrg = (short*)temp->ptr(3 * radiusV + 3 * range.start + 1) + 4 * (radiusH / 4 + 1);
				short* sptrr = (short*)temp->ptr(3 * radiusV + 3 * range.start + 2) + 4 * (radiusH / 4 + 1);

				short* dptr = dest->ptr<short>(range.start);

				for (int i = range.start; i != range.end; i++, sptrr += sstep, sptrg += sstep, sptrb += sstep, dptr += dstep)
				{
					for (int j = 0; j < size.width; j++)
					{
						const short* sptrrj = sptrr + j;
						const short* sptrgj = sptrg + j;
						const short* sptrbj = sptrb + j;

						short r0 = sptrrj[0];
						short g0 = sptrgj[0];
						short b0 = sptrbj[0];

						float sum_r = 0.f, sum_b = 0.f, sum_g = 0.f;
						float wsum = 0.f;
						for (int k = 0; k < maxk; k++)
						{
							float r = sptrrj[space_ofs[k]], g = sptrgj[space_ofs[k]], b = sptrbj[space_ofs[k]];
						...
						wsum = 1.f / wsum;
						dptr[3 * j + 0] = saturate_cast<short>(sum_b * wsum);
						dptr[3 * j + 1] = saturate_cast<short>(sum_g * wsum);
						dptr[3 * j + 2] = saturate_cast<short>(sum_r * wsum);
					}
				}
			}
		}
	...
	void binaryWeightedRangeFilter_16S(const Mat& src, Mat& dst, Size kernelSize, float threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int i, j, maxk;
		...
		CV_Assert((src.type() == CV_16SC1 || src.type() == CV_16SC3) &&
			src.type() == dst.type() && src.size() == dst.size());
		...
		int dpad = (8 - src.cols % 8) % 8;
		int spad = dpad + (8 - (2 * radiusH) % 8) % 8;
		if (spad < 8) spad += 8;
		int lpad = 8 * (radiusH / 8 + 1) - radiusH;
		int rpad = spad - lpad;
		if (cn == 1)
		{
			copyMakeBorder(src, temp, radiusV, radiusV, radiusH + lpad, radiusH + rpad, borderType);
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		binaryWeightedRangeFilter_16S_InvokerAVX body(dest, temp, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}


	void binaryWeightedRangeFilter(cv::InputArray src_, cv::OutputArray dst_, int D, float threshold, int method, int borderType)
	{
		binaryWeightedRangeFilter(src_, dst_, Size(D, D), threshold, method, borderType);
	}

	void binaryWeightedRangeFilter(cv::InputArray src_, cv::OutputArray dst_, Size kernelSize, float threshold, int method, int borderType)
	{
		...
		if (method == FILTER_SLOWEST)
		{
			if (src.depth() == CV_8U)
			{
				binaryWeightedRangeFilterNONSSE_8u(src, dst, kernelSize, (uchar)threshold, borderType, false);
			}
			else if (src.depth() == CV_32F)
			{
				binaryWeightedRangeFilterNONSSE_32f(src, dst, kernelSize, threshold, borderType, false);
			}
			else
			{
		...
		else if (method == FILTER_CIRCLE || method == FILTER_DEFAULT)
		{
			if (src.depth() == CV_8U)
			{
				binaryWeightedRangeFilter_8u(src, dst, kernelSize, (uchar)threshold, borderType, false);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				binaryWeightedRangeFilter_16S(src, dst, kernelSize, threshold, borderType, false);
			}
			else if (src.depth() == CV_32F)
			{
				binaryWeightedRangeFilter_32f(src, dst, kernelSize, threshold, borderType, false);
			}
		}
		else if (method == FILTER_RECTANGLE)
		{
			if (src.depth() == CV_8U)
			{
				binaryWeightedRangeFilter_8u(src, dst, kernelSize, (uchar)threshold, borderType, true);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				binaryWeightedRangeFilter_16S(src, dst, kernelSize, threshold, borderType, true);
			}
			else if (src.depth() == CV_32F)
			{
				binaryWeightedRangeFilter_32f(src, dst, kernelSize, threshold, borderType, true);
			}
		}
		else if (method == FILTER_SEPARABLE)
		{
			...
			{
				binaryWeightedRangeFilterSP_8u(src, dst, kernelSize, (uchar)threshold, borderType);
			}
			else if (src.type() == CV_MAKE_TYPE(CV_32F, src.channels()))
			{
				binaryWeightedRangeFilterSP_32f(src, dst, kernelSize, threshold, borderType);
			}
		}
	}
#pragma endregion

#pragma region jointBinaryWeightedRangeFilter
	class JointBinaryWeightedRangeFilter_8u_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		JointBinaryWeightedRangeFilter_8u_InvokerSSE4(Mat& _dest, const Mat& _temp, const Mat& guide, int _radiusH, int _radiusV, int _maxk,
	...
	void jointbinaryWeightedRangeFilter_8u(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, uchar threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		JointBinaryWeightedRangeFilter_8u_InvokerSSE4 body(dest, temp, gim, radiusH, radiusV, maxk, space_ofs, space_ofsj, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	void jointBinaryWeightedRangeFilterSP_8u(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, uchar threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else jointbinaryWeightedRangeFilter_8u(src, guide, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			jointbinaryWeightedRangeFilter_8u(dst, guide, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}

	class JointBinaryWeightedRangeFilter_32f_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		JointBinaryWeightedRangeFilter_32f_InvokerSSE4(Mat& _dest, const Mat& _temp, const Mat& _guide, int _radiusH, int _radiusV, int _maxk,
	...
	void jointbinaryWeightedRangeFilter_32f(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, float threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int cng = guide.channels();
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		JointBinaryWeightedRangeFilter_32f_InvokerSSE4 body(dest, temp, gim, radiusH, radiusV, maxk, space_ofs, space_ofsj, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	void jointBinaryWeightedRangeFilterSP_32f(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, float threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else jointbinaryWeightedRangeFilter_32f(src, guide, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			jointbinaryWeightedRangeFilter_32f(dst, guide, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}


	void jointBinaryWeightedRangeFilter(cv::InputArray src_, cv::InputArray guide_, cv::OutputArray dst_, int D, float threshold, int method, int borderType)
	{
		jointBinaryWeightedRangeFilter(src_, guide_, dst_, Size(D, D), threshold, method, borderType);
	}

	void jointBinaryWeightedRangeFilter(cv::InputArray src_, cv::InputArray guide_, cv::OutputArray dst_, Size kernelSize, float threshold, int method, int borderType)
	{
		...
		if (method == FILTER_CIRCLE || method == FILTER_DEFAULT)
		{
			if (src.depth() == CV_8U)
			{
				jointbinaryWeightedRangeFilter_8u(src, guide, dst, kernelSize, (uchar)threshold, borderType, false);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				Mat srcf, guidef;
				...
				jointbinaryWeightedRangeFilter_32f(srcf, guidef, destf, kernelSize, threshold, borderType, false);
				destf.convertTo(dst, src.depth(), 1.0, 0.0);
			}
			else if (src.depth() == CV_32F)
			{
				jointbinaryWeightedRangeFilter_32f(src, guide, dst, kernelSize, threshold, borderType, false);
			}
		}
		else if (method == FILTER_RECTANGLE)
		{
			if (src.depth() == CV_8U)
			{
				jointbinaryWeightedRangeFilter_8u(src, guide, dst, kernelSize, (uchar)threshold, borderType, true);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				Mat srcf, guidef;
				...
				jointbinaryWeightedRangeFilter_32f(srcf, guidef, destf, kernelSize, threshold, borderType, true);
				destf.convertTo(dst, src.depth(), 1.0, 0.0);
			}
			else if (src.depth() == CV_32F)
			{
				jointbinaryWeightedRangeFilter_32f(src, guide, dst, kernelSize, threshold, borderType, true);
			}
		}
		else if (method == FILTER_SEPARABLE)
		{
			if (src.depth() == CV_8U)
			{
				jointBinaryWeightedRangeFilterSP_8u(src, guide, dst, kernelSize, (uchar)threshold, borderType);
			}
			else if (src.depth() == CV_32F)
			{
				jointBinaryWeightedRangeFilterSP_32f(src, guide, dst, kernelSize, threshold, borderType);
			}
		}
	}
#pragma endregion

#pragma region centerReplacedBinaryWeightedRangeFilter

	class CenterReplacedbinaryWeightedRangeFilter_8u_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		CenterReplacedbinaryWeightedRangeFilter_8u_InvokerSSE4(Mat& _dest, const Mat& _temp, const Mat& _center, int _radiusH, int _radiusV, int _maxk,
			int* _space_ofs, uchar _threshold) :
	...
	void centerReplacedbinaryWeightedRangeFilter_8u(const Mat& src, const Mat& center, Mat& dst, Size kernelSize, uchar threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int i, j, maxk;
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		CenterReplacedbinaryWeightedRangeFilter_8u_InvokerSSE4 body(dest, temp, centert, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}

	void centerReplacedBinaryWeightedRangeFilterSP_8u(const Mat& src, const Mat& center, Mat& dst, Size kernelSize, uchar threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else centerReplacedbinaryWeightedRangeFilter_8u(src, center, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			centerReplacedbinaryWeightedRangeFilter_8u(dst, center, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}


	class CenterReplacedbinaryWeightedRangeFilter_32f_InvokerSSE4 : public cv::ParallelLoopBody
	{
	public:
		CenterReplacedbinaryWeightedRangeFilter_32f_InvokerSSE4(Mat& _dest, const Mat& _temp, const Mat& _center, int _radiusH, int _radiusV, int _maxk,
			int* _space_ofs, float _threshold) :
			temp(&_temp), center(&_center), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV),
			maxk(_maxk), space_ofs(_space_ofs), threshold(_threshold)
		{
	...
	void centerReplacedbinaryWeightedRangeFilter_32f(const Mat& src, const Mat& center, Mat& dst, Size kernelSize, float threshold, int borderType, bool isRectangle)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		int cn = src.channels();
		int i, j, maxk;
		...
		Mat dest = Mat::zeros(Size(src.cols + dpad, src.rows), dst.type());
		CenterReplacedbinaryWeightedRangeFilter_32f_InvokerSSE4 body(dest, temp, centert, radiusH, radiusV, maxk, space_ofs, threshold);
		parallel_for_(Range(0, size.height), body);
		Mat(dest(Rect(0, 0, dst.cols, dst.rows))).copyTo(dst);
	}


	void centerReplacedBinaryWeightedRangeFilterSP_32f(const Mat& src, const Mat& center, Mat& dst, Size kernelSize, float threshold, int borderType)
	{
		if (kernelSize.width <= 1) src.copyTo(dst);
		else centerReplacedbinaryWeightedRangeFilter_32f(src, center, dst, Size(kernelSize.width, 1), threshold, borderType, false);

		if (kernelSize.width > 1)
			centerReplacedbinaryWeightedRangeFilter_32f(dst, center, dst, Size(1, kernelSize.height), threshold, borderType, false);
	}

	void centerReplacedBinaryWeightedRangeFilter(const Mat& src, const Mat& center, Mat& dst, Size kernelSize, float threshold, int method, int borderType)
	{
		...
		if (method == FILTER_CIRCLE || method == FILTER_DEFAULT)
		{
			if (src.depth() == CV_8U)
			{
				centerReplacedbinaryWeightedRangeFilter_8u(src, center, dst, kernelSize, (uchar)threshold, borderType, false);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				...
				centerReplacedbinaryWeightedRangeFilter_32f(srcf, centerf, destf, kernelSize, threshold, borderType, false);
				destf.convertTo(dst, src.depth(), 1.0);
			}
			else if (src.depth() == CV_32F)
			{
				centerReplacedbinaryWeightedRangeFilter_32f(src, center, dst, kernelSize, threshold, borderType, false);
			}
		}
		else if (method == FILTER_RECTANGLE)
		{
			if (src.depth() == CV_8U)
			{
				centerReplacedbinaryWeightedRangeFilter_8u(src, center, dst, kernelSize, (uchar)threshold, borderType, true);
			}
			else if (src.depth() == CV_16S || src.depth() == CV_16U)
			{
				Mat srcf; src.convertTo(srcf, CV_32F);
				...
				centerReplacedbinaryWeightedRangeFilter_32f(srcf, centerf, destf, kernelSize, threshold, borderType, true);
				destf.convertTo(dst, src.depth(), 1.0, 0.0);
			}
			else if (src.depth() == CV_32F)
			{
				centerReplacedbinaryWeightedRangeFilter_32f(src, center, dst, kernelSize, threshold, borderType, true);
			}
		}
		else if (method == FILTER_SEPARABLE)
		{
			if (src.type() == CV_MAKE_TYPE(CV_8U, src.channels()))
			{
				centerReplacedBinaryWeightedRangeFilterSP_8u(src, center, dst, kernelSize, (uchar)threshold, borderType);
			}
			else if (src.type() == CV_MAKE_TYPE(CV_32F, src.channels()))
			{
				centerReplacedBinaryWeightedRangeFilterSP_32f(src, center, dst, kernelSize, threshold, borderType);
			}
		}
	}
#pragma endregion
