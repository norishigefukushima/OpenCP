...
*/
void deriche_gaussian_conv_sse_float(deriche_coeffs c, float *dest, float *buffer, const float *src, int N, int stride)
{
	const int stride_2 = stride * 2;
	const int stride_3 = stride * 3;
	const int stride_4 = stride * 4;
	const int stride_N = stride * N;
	float *y_causal, *y_anticausal;
	long i, n;
...
void deriche_gaussian_conv_image(deriche_coeffs c, num *dest, num *buffer, const num *src, int width, int height, int num_channels)
{
	...
	for (channel = 0; channel < num_channels; ++channel)
	{
		...
		for (x = 0; x < width; ++x)
		{
			deriche_gaussian_conv_sse_float(c, dest + x, buffer, dest + x, height, width);
		}
