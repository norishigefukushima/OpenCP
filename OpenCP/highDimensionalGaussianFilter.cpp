...
#pragma omp parallel for schedule(dynamic)
	...
	void highDimensionalGaussianFilter_Dim(const cv::Mat src, const cv::Mat guide, cv::Mat& dst, const cv::Size ksize, const float sigma_range, const float sigma_space, const int border)
	{
		const int r = ksize.height / 2;
		Mat srcborder; copyMakeBorder(src, srcborder, r, r, r, r, border);
		Mat guideborder; copyMakeBorder(guide, guideborder, r, r, r, r, border);
...
#pragma omp parallel for schedule(dynamic)
	...
	void highDimensionalGaussianFilter(InputArray src, InputArray guide, OutputArray dst, const cv::Size ksize, const float sigma_range, const float sigma_space, const int border)
	{
		dst.create(src.size(), src.type());
		switch (guide.channels())
		{
		case 1:
			highDimensionalGaussianFilter_<1>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 2:
			highDimensionalGaussianFilter_<2>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 3:
			highDimensionalGaussianFilter_<3>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 4:
			highDimensionalGaussianFilter_<4>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 5:
			highDimensionalGaussianFilter_<5>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 6:
			highDimensionalGaussianFilter_<6>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 7:
			highDimensionalGaussianFilter_<7>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 8:
			highDimensionalGaussianFilter_<8>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		case 9:
			highDimensionalGaussianFilter_<9>(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		default:
			highDimensionalGaussianFilter_Dim(src.getMat(), guide.getMat(), dst.getMat(), ksize, sigma_range, sigma_space, border); break;
		}	
	}
}