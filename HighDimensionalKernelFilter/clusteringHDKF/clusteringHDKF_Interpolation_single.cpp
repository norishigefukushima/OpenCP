...
namespace cp
{
	void ClusteringHDKF_InterpolationSingle::setIsUseLocalMu(const bool flag)
	{
		this->isUseLocalMu = flag;
	}

	void ClusteringHDKF_InterpolationSingle::setIsUseLocalStatisticsPrior(const bool flag)
	{
		this->isUseLocalStatisticsPrior = flag;
	}

	void ClusteringHDKF_InterpolationSingle::setDeltaLocalStatisticsPrior(const float delta)
	{
		this->delta = delta;
	}

	void ClusteringHDKF_InterpolationSingle::alloc(cv::Mat& dst)
	{
	...
	void ClusteringHDKF_InterpolationSingle::computeWandAlpha(const std::vector<cv::Mat>& guide)
	{
	...
	//isUseLocalMu=false, without local mu
	void ClusteringHDKF_InterpolationSingle::computeIndex(const std::vector<cv::Mat>& guide, const std::vector<cv::Mat>& guideRes)
	{
	...
	template<int use_fmath>
	void ClusteringHDKF_InterpolationSingle::computeAlpha(const std::vector<cv::Mat>& guide, const int k)
	{
		bool isROI = true;
	...
	template<int use_fmath>
	void ClusteringHDKF_InterpolationSingle::computeW(const std::vector<cv::Mat>& src, std::vector<cv::Mat>& vecW)
	{
	...
	void ClusteringHDKF_InterpolationSingle::setMergingNumerDenomMat(vector<Mat>& dest, const int k, const int upsampleSize)
	{
	...
	void ClusteringHDKF_InterpolationSingle::mergeRecomputeAlphaForUsingMu(std::vector<cv::Mat>& src, const int k)
	{
		if (isJoint)
		{
		...
		setMergingNumerDenomMat(intermat, k, downSampleImage);
	...
	template<int use_fmath, const bool isInit>
	void ClusteringHDKF_InterpolationSingle::mergeRecomputeAlphaForUsingNLMMu(std::vector<cv::Mat>& src, const int k)
	{
		if (isJoint)
		{
			//std::cout << "ConstantTimeHDGF_InterpolationSingle::recomputeAlpha must be src==guide" << std::endl;
	...
	void ClusteringHDKF_InterpolationSingle::mergeRecomputeAlphaForUsingMuPCA(std::vector<cv::Mat>& guide, const int k)
	{
	...
	//without mu<int use_fmath, const bool isInit>
	template<int use_fmath, const bool isInit>
	void ClusteringHDKF_InterpolationSingle::mergeRecomputeAlpha(const std::vector<cv::Mat>& guide, const int k)
	{
	...
	void ClusteringHDKF_InterpolationSingle::mergePreComputedAlpha(const int k, const bool isInit)
	{
		//merge
		const int wk = (isUsePrecomputedWforeachK) ? k : 0;
		vector<Mat> intermat(channels + 1);
	...
	void ClusteringHDKF_InterpolationSingle::merge(const int k, const bool isInit)
	{
		//merge
	...
	void ClusteringHDKF_InterpolationSingle::normalize(cv::Mat& dst)
	{
		if (channels == 1)
		{
			for (int y = boundaryLength; y < img_size.height - boundaryLength; y++)
	...
	void ClusteringHDKF_InterpolationSingle::body(const std::vector<cv::Mat>& src, cv::Mat& dst, const std::vector<cv::Mat>& guide)
	{
		{
			//cp::Timer t("alloc");
			alloc(dst);
