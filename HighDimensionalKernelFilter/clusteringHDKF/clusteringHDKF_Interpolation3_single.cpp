...
#pragma region setter
	void ClusteringHDKF_Interpolation3Single::setIsUseLocalMu(const bool flag)
	{
		this->isUseLocalMu = flag;
	}

	void ClusteringHDKF_Interpolation3Single::setIsUseLocalStatisticsPrior(const bool flag)
	{
		this->isUseLocalStatisticsPrior = flag;
	}

	void ClusteringHDKF_Interpolation3Single::setDeltaLocalStatisticsPrior(const float delta)
	{
		this->delta = delta;
	}
#pragma endregion

	void ClusteringHDKF_Interpolation3Single::alloc(cv::Mat& dst)
	{
		...
		if (vecW.size() != K || vecW[0].size() != img_size / downSampleImage)
		{
			...
			for (int i = 0; i < K; i++)
			{
				...
				vmap[i].create(img_size / downSampleImage, CV_32FC1);
			}
		}
	...
	void ClusteringHDKF_Interpolation3Single::computeWandAlpha(const std::vector<cv::Mat>& guide)
	{
	...
	STATIC_INLINE void _mm256_argmin2nd_ps(const __m256 src, __m256& minval, __m256& argment, const __m256 minval1st, const float index)
	{
		const __m256 mask1 = _mm256_cmp_ps(src, minval1st, 4);
		const __m256 mask = _mm256_and_ps(_mm256_cmp_ps(src, minval, 2), mask1);
	...
	//isUseLocalMu=false, without local mu
	void ClusteringHDKF_Interpolation3Single::computeIndex(const std::vector<cv::Mat>& guide, const std::vector<cv::Mat>& guideRes)
	{
		...
		const __m256 mcoef = _mm256_set1_ps(coeff);
		const double s = 3.0;
		const float coeff2 = float(-1.0 / (2.0 * s * s));
		const __m256 mcoef2 = _mm256_set1_ps(coeff2);
		...
		else if (guide_channels == 3)
		{
			...
			if constexpr (method == 0)
			{
				...
				AutoBuffer<float*> vptr(K);
				for (int k = 0; k < K; k++)
				{
					wptr[k] = wmap[k].ptr<float>();
					vptr[k] = vmap[k].ptr<float>();
				}
				...
				for (int n = 0; n < IMSIZE8; n++)
				{
					...
					for (int k = 0; k < K; k++)
					{
						...
						mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
						const __m256 mexp = v_exp_ps<true>(_mm256_mul_ps(mcoef, mdiff));
						_mm256_store_ps(vptr[k] + 8 * n, mexp);
					...
					for (int k = 0; k < K; k++)
					{
						_mm256_store_ps(wptr[k] + 8 * n, _mm256_andnot_ps(_mm256_set1_ps(-1.f), _mm256_cmp_ps(argment, _mm256_set1_ps(float(k)), 0)));
					}

					/*__m256 mdiffmax2 = _mm256_set1_ps(FLT_MAX);
					__m256 argment2 = _mm256_setzero_ps();

					{
						...
						mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
						const __m256 mexp = v_exp_ps<true>(_mm256_mul_ps(mcoef, mdiff));
						_mm256_store_ps(wptr[k] + 8 * n, mexp);
						//const __m256 malpha = );
						mdiffmin = _mm256_min_ps(mdiffmin, mdiff);
						mdiffmax2 = mdiff;
					}
					for (int k = 1; k < K; k++)
					{
						...
						//const __m256 malpha = );
						mdiffmin = _mm256_min_ps(mdiffmin, mdiff);
						_mm256_argmin2nd_ps(mdiff, mdiffmax2, argment2, mdiffmax, float(k));
					}
					...
					for (int n = 0; n < 8; n++)
					{
						const int v0 = int(argment.m256_f32[n]);
						const int v1 = int(argment2.m256_f32[n]);
						min00.m256_f32[n] = mc0[v0].m256_f32[0];
						min01.m256_f32[n] = mc1[v0].m256_f32[0];
						min02.m256_f32[n] = mc2[v0].m256_f32[0];
						min10.m256_f32[n] = mc0[v1].m256_f32[0];
						min11.m256_f32[n] = mc1[v1].m256_f32[0];
						min12.m256_f32[n] = mc2[v1].m256_f32[0];
					}
					__m256 msub = _mm256_sub_ps(*im0, min00);
					__m256 mdiff = _mm256_mul_ps(msub, msub);
					msub = _mm256_sub_ps(*im1, min01);
					mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
					msub = _mm256_sub_ps(*im2, min02);
					mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
					//mdiff = _mm256_sqrt_ps(mdiff);

					msub = _mm256_sub_ps(*im0, min10);
					__m256 mdiff2 = _mm256_mul_ps(msub, msub);
					msub = _mm256_sub_ps(*im1, min11);
					mdiff2 = _mm256_fmadd_ps(msub, msub, mdiff2);
					msub = _mm256_sub_ps(*im2, min12);
					mdiff2 = _mm256_fmadd_ps(msub, msub, mdiff2);
					//mdiff2 = _mm256_sqrt_ps(mdiff2);

					__m256 msum2 = _mm256_add_ps(mdiff, mdiff2);
					mdiff = _mm256_sub_ps(_mm256_set1_ps(1.f),_mm256_div_ps(mdiff, msum2));
					mdiff2 = _mm256_sub_ps(_mm256_set1_ps(1.f), _mm256_div_ps(mdiff2, msum2));
					for (int k = 0; k < K; k++)
					{
						__m256 mask = _mm256_cmp_ps(_mm256_set1_ps(k), argment, _CMP_EQ_OQ);
						__m256 mask2 = _mm256_cmp_ps(_mm256_set1_ps(k), argment2, _CMP_EQ_OQ);
						__m256 v = _mm256_add_ps(_mm256_blendv_ps(_mm256_setzero_ps(), mdiff, mask), _mm256_blendv_ps(_mm256_setzero_ps(), mdiff2, mask2));
						//cout << k << endl;
						//print_m256(v);
						//print_m256(argment);
						//print_m256(argment2);

						_mm256_store_ps(vptr[k] + 8 * n, v);
					}*/
					//getchar();
					/*
					for (int k = 0; k < K; k++)
					{
						...
						const __m256 mexp = v_exp_ps<true>(_mm256_mul_ps(mcoef2, _mm256_sub_ps(mdiff, mdiffmin)));
						msum = _mm256_add_ps(msum, mexp);
					}
					for (int k = 0; k < K; k++)
					{
						__m256 msub = _mm256_sub_ps(*im0, mc0[k]);
						...
						//_mm256_store_ps(vptr[k] + 8 * n, mexp);
						_mm256_store_ps(vptr[k] + 8 * n, _mm256_div_ps(mexp, msum));
						//_mm256_store_ps(vptr[k] + 8 * n, _mm256_andnot_ps(_mm256_set1_ps(-1.f), _mm256_cmp_ps(argment, _mm256_set1_ps(float(k)), 0)));
					}
					*/
					*idx++ = argment;
					im0++; im1++; im2++;
	...
	template<int use_fmath>
	void ClusteringHDKF_Interpolation3Single::computeAlpha(const std::vector<cv::Mat>& guide, const int k)
	{
		bool isROI = true;
	...
	template<int use_fmath>
	void ClusteringHDKF_Interpolation3Single::computeW(const std::vector<cv::Mat>& src, std::vector<cv::Mat>& vecW)
	{
	...
	void ClusteringHDKF_Interpolation3Single::split_blur(const int k, const bool isUseFmath, const bool isUseLSP)
	{
		const Size size = img_size / downSampleImage;
		const int imsize = size.area();
		const int IMSIZE8 = imsize / 8;
...
#pragma region merge
	void ClusteringHDKF_Interpolation3Single::mergeNumerDenomMat(vector<Mat>& dest, const int k, const int upsampleSize)
	{
	...
	void ClusteringHDKF_Interpolation3Single::mergeRecomputeAlphaForUsingMu(std::vector<cv::Mat>& src, const int k)
	{
		if (isJoint)
		{
	...
	template<int use_fmath, const bool isInit>
	void ClusteringHDKF_Interpolation3Single::mergeRefineAlpha(std::vector<cv::Mat>& src, std::vector<cv::Mat>& refine, const int k)
	{
		if (isJoint)
		{
			//std::cout << "ConstantTimeHDGF_InterpolationSingle::recomputeAlpha must be src==guide" << std::endl;
		...
		//if (channels == 3)
		{
			for (int y = boundaryLength; y < h - boundaryLength; y++)
			{
				...
				const __m256* src1 = (const __m256*)src[1].ptr<float>(y, boundaryLength);
				const __m256* src2 = (const __m256*)src[2].ptr<float>(y, boundaryLength);
				const __m256* ref0 = (const __m256*)refine[0].ptr<float>(y, boundaryLength);
				const __m256* ref1 = (const __m256*)refine[1].ptr<float>(y, boundaryLength);
				const __m256* ref2 = (const __m256*)refine[2].ptr<float>(y, boundaryLength);
				const __m256* inter0 = (const __m256*)intermat[0].ptr<float>(y, boundaryLength);
				const __m256* inter1 = (const __m256*)intermat[1].ptr<float>(y, boundaryLength);
				const __m256* inter2 = (const __m256*)intermat[2].ptr<float>(y, boundaryLength);
				const __m256* interw = (const __m256*)intermat[3].ptr<float>(y, boundaryLength);
				...
				for (int x = boundaryLength; x < w - boundaryLength; x += 8)
				{
					__m256 msub = _mm256_sub_ps(*ref0, *src0);
					__m256 mdiff = _mm256_mul_ps(msub, msub);
					msub = _mm256_sub_ps(*ref1, *src1);
					mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
					msub = _mm256_sub_ps(*ref2, *src2);
					mdiff = _mm256_fmadd_ps(msub, msub, mdiff);
	...
	template<int use_fmath, const bool isInit>
	void ClusteringHDKF_Interpolation3Single::mergeRecomputeAlphaForUsingNLMMu(std::vector<cv::Mat>& src, const int k)
	{
		if (isJoint)
		{
			//std::cout << "ConstantTimeHDGF_InterpolationSingle::recomputeAlpha must be src==guide" << std::endl;
	...
	void ClusteringHDKF_Interpolation3Single::mergeRecomputeAlphaForUsingMuPCA(std::vector<cv::Mat>& guide, const int k)
	{
	...
	template<int use_fmath, const bool isInit>
	void ClusteringHDKF_Interpolation3Single::mergeRecomputeAlpha(const std::vector<cv::Mat>& guide, const int k)
	{
		...
		const double s = 1.0;
		const float coeff = float(-1.0 / (2.0 * s * s));
		const __m256 mcoef = _mm256_set1_ps(coeff);
		...
		else if (channels == 3 && guide_channels == 3)
		{
			...
			for (int y = boundaryLength; y < img_size.height - boundaryLength; y++)
			{
				...
				__m256* idx = (__m256*)vmap[k].ptr<float>(y, boundaryLength);
				for (int x = boundaryLength; x < img_size.width - boundaryLength; x += 8)
				{
					__m256 msub = _mm256_sub_ps(*im0++, mc0);
					__m256 mdiff = _mm256_mul_ps(msub, msub);
	...
	void ClusteringHDKF_Interpolation3Single::mergePreComputedAlpha(const int k, const bool isInit)
	{
		//merge
		const int wk = (isUsePrecomputedWforeachK) ? k : 0;
		vector<Mat> intermat(channels + 1);
	...
	void ClusteringHDKF_Interpolation3Single::merge(const int k, const bool isInit)
	{
		//merge
...
#pragma endregion
	...
	void ClusteringHDKF_Interpolation3Single::normalize(cv::Mat& dst)
	{
		if (channels == 1)
		{
			for (int y = boundaryLength; y < img_size.height - boundaryLength; y++)
	...
	void ClusteringHDKF_Interpolation3Single::body(const std::vector<cv::Mat>& src, cv::Mat& dst, const std::vector<cv::Mat>& guide)
	{
		{
			//cp::Timer t("alloc");
			alloc(dst);
		...
		if (isRefine)
		{
			...
			{
				//cp::Timer t("normal");
				refine[0].create(numer[0].size(), CV_32F);
				refine[1].create(numer[0].size(), CV_32F);
				refine[2].create(numer[0].size(), CV_32F);
				for (int y = boundaryLength; y < img_size.height - boundaryLength; y++)
				{
					__m256* ref0 = (__m256*)refine[0].ptr<float>(y, boundaryLength);
					__m256* ref1 = (__m256*)refine[1].ptr<float>(y, boundaryLength);
					__m256* ref2 = (__m256*)refine[2].ptr<float>(y, boundaryLength);

					const __m256* numer0 = (__m256*)numer[0].ptr<float>(y, boundaryLength);
					const __m256* numer1 = (__m256*)numer[1].ptr<float>(y, boundaryLength);
					const __m256* numer2 = (__m256*)numer[2].ptr<float>(y, boundaryLength);
					const __m256* denom_ = (__m256*)denom.ptr<float>(y, boundaryLength);
					for (int x = boundaryLength; x < img_size.width - boundaryLength; x += 8)
					{
						const __m256 mdenom__ = *denom_++;

						*ref0 = _mm256_div_avoidzerodiv_ps(*numer0++, mdenom__);
						*ref1 = _mm256_div_avoidzerodiv_ps(*numer1++, mdenom__);
						*ref2 = _mm256_div_avoidzerodiv_ps(*numer2++, mdenom__);
						ref0++; ref1++; ref2++;
						//mb = _mm256_max_ps(_mm256_min_ps(mb, m255), _mm256_setzero_ps());
						//mg = _mm256_max_ps(_mm256_min_ps(mg, m255), _mm256_setzero_ps());
						//mr = _mm256_max_ps(_mm256_min_ps(mr, m255), _mm256_setzero_ps());
			...
			//Mat v; cv::merge(refine, v); imshowScale("aa", v); waitKey();
			switch (channels)
			{
			case 1:
			{
				for (int k = 0; k < K; k++)
				{
					...
					if (k == 0)
					{
						if (isUseFmath) mergeRefineAlpha<1, true>(vsrc, refine, k);
						else mergeRefineAlpha<0, true>(vsrc, refine, k);
					}
					else
					{
						if (isUseFmath) mergeRefineAlpha<1, false>(vsrc, refine, k);
						else mergeRefineAlpha<0, false>(vsrc, refine, k);
					}
				}
				break;
			}
			case 3:
			{
				for (int k = 0; k < K; k++)
				{
					...
					if (k == 0)
					{
						...
						//if (isUseFmath) mergeRecomputeAlphaForUsingMu<1, true>(vsrc, k);
						//else mergeRecomputeAlphaForUsingMu<0, true>(vsrc, k);
						if (isUseFmath) mergeRefineAlpha<1, true>(vguide, refine, k);
						else mergeRefineAlpha<0, true>(vguide, refine, k);
					}
					else
					{
						//if (isUseFmath)mergeRecomputeAlpha<1, false>(vguide, k);
						//else mergeRecomputeAlpha<0, false>(vguide, k);
						//if (isUseFmath) mergeRecomputeAlphaForUsingMu<1, false>(vguide, k);
						//else mergeRecomputeAlphaForUsingMu<0, false>(vguide, k);
						if (isUseFmath) mergeRefineAlpha<1, false>(vguide, refine, k);
						else mergeRefineAlpha<0, false>(vguide, refine, k);
					}
				}
				break;
			}
