...
#pragma region window setup
	...
	int sigma_r_ = 1000; createTrackbar("sigma_range", "trackbar", &sigma_r_, 1000);
...
#pragma region variable
	...
	int count = 0, ct = 0;
	float time_compre = 0.f, time_linear = 0.f, time_tile = 0.f, error;

	Scalar mean, stddev;
#pragma endregion
	bool isFile = false;//false

	vector<double> PSNR_Compre_Sum(100);
	vector<double> PSNR_FastLLF_Sum(100);
	if (isFile)
	{
		for (int i = 0; i < PSNR_Compre_Sum.size(); i++)
		{
			PSNR_Compre_Sum[i] = 0.0;
			PSNR_FastLLF_Sum[i] = 0.0;
		}
	}
...
#pragma region setup
			...
			if (ucim.isUpdate(im, Color))
			{
				...
				if (isFile) src = imread(filename[num[i]], Color);
...
#pragma region main processing
			...
			//unnormalizedBilateralFilterMulti(srcf, destUNBFM, (int)ceil(sigma_s * 3.f), sigma_r, sigma_s, level, isEnhance);
			//unnormalizedBilateralFilter(srcf, destUNBF, (int)ceil(sigma_s * level * 3.f), sigma_r, sigma_s * level, isEnhance);
#pragma endregion
#endif
...
#pragma region PSNR
			...
			if (isPyramid)
			{
				...
				{ cp::imshowScale("PyramidTileFourier", destTilePyramidFourier); cp::diffshow("PyTileFourier-PyNaive", destTilePyramidFourier, destPyramidNaive, 10000.0); }
			...
			//ci("DoGComp-PyNN   | %5.2f dB", cp::getPSNR(destDoGCompressive, destPyramidNN, bb));
			//ci("DoGComp-UNBF   | %5.2f dB", cp::getPSNR(destDoGCompressive, destUNBFM, bb));
#pragma endregion
