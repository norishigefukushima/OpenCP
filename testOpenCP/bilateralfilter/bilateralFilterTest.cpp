...
void testAccuracy(Mat& src, int d, float sigma_color, float sigma_space)
{
	Mat destOpenCV;
	cv::bilateralFilter(src, destOpenCV, d, sigma_color, sigma_space, BORDER_REPLICATE);

	Mat dest;
	cp::bilateralFilter(src, dest, Size(d, d), sigma_color, sigma_space, FILTER_CIRCLE, BORDER_REPLICATE);

	diffshow("diff", dest, destOpenCV, 30);
	cout << "PSNR" << ": " << cv::PSNR(destOpenCV, dest) << "dB" << endl;
...
void guiBilateralFilterTest(Mat& src)
{
	...
	while (key != 'q')
	{
		...
		if (key == 't')
		{
			testAccuracy(src, d, sigma_color, sigma_space);
		}
